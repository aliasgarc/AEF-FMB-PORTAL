...
router.get('/:itsId', async (req, res) => {
  ...
  try {
    ...
    const paymentsResult = await db.query(
      'SELECT receipt_no, amt_rcv, amt_pending, payment_mode, received_date, payment_refrence, mobile_no, created_at, updated_at FROM fmb_payment_tbl WHERE hof_its = CAST($1 AS INTEGER) ORDER BY received_date DESC',
      [itsId]
    );
